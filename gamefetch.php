	...
	for($j=0;$j<8;$j++){
		for($k=0;$k<31;$k++){
			...
			foreach($xml as $games){
				...
				@$gamequery = 'INSERT INTO games_'.$year[$i].' (game_id, game_id_number, calendar_event_id, game_group, game_type, series, series_num, game_pk, event_time, status, inning, venue, venue_id, away_code, away_team_id, away_name_abbrev, away_team_name, away_score, home_code, home_team_id, home_name_abbrev, home_team_name, home_score) VALUES ("'.$game_id.'","'.$id.'","'.$calendar_event_id.'","'.$group.'","'.$game_type.'","'.$series.'","'.$series_num.'","'.$game_pk.'","'.$event_time.'","'.$status.'","'.$inning.'","'.$venue.'","'.$venue_id.'","'.$away_code.'","'.$away_team_id.'","'.$away_name_abbrev.'","'.$away_team_name.'","'.$away_score.'","'.$home_code.'","'.$home_team_id.'","'.$home_name_abbrev.'","'.$home_team_name.'","'.$home_score.'")';
